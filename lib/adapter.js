(function(window) {
  ...
  var adapter = {
     ...
     * @returns {RegExp}
     */
    createTestFileRegexp: function(testFileRegex, testFileSuffix) {
      if (testFileRegex) {
          return new RegExp(testFileRegex.replace(/\./g, '\\.'));
      }
      else if (testFileSuffix) {
    ...
    run: function(karma, System, Promise) {
      ...
      var testFileRegexp = adapter.createTestFileRegexp(karma.config.systemjs.testFileRegex, karma.config.systemjs.testFileSuffix);
