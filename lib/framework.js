...
var initSystemjsFramework = function (config, logger) {
  ...
  // A SystemJS config must have been specified
  if (!systemjsConfig.configFile && !systemjsConfig.config) {
    log.error('The SystemJS config must be specified in the karma config at `systemjs.configFile` and/or `systemjs.config`.');
    throw new Error('No SystemJS config found');
  }
